...
def buscar_chats():
    ...
    #print(len(driver.find_elements(By.CLASS_NAME,"zaKsw")))# si la longitud es 0 es porque tengo chat abierto, si es dif de 0 es porque  no hay chat abierto
...
def procesar_mensaje(mensaje :str):
    ...
    respuesta = preparar_respuesta(mensaje)         # CONEXIÓN CON EL CHATBOT

    if ".png" in respuesta:
        preparar_imagen(respuesta)                  # RESPUESTA CON IMAGEN PNG DE RECOMENDACIÓN

    else:
        chat_box.send_keys(respuesta, Keys.ENTER)   # RESPUESTA DESDE EL BOT
        sleep(2)
...
def whatsapp_bot_init():
    ...
    while esperando== 1:
        ...
        sleep(10) # Aquí también depende del tiempo de arranque del navegador. No es muy práctico pero es funcional.
